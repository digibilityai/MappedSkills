...
const ROWS = [
  ...
  { label: 'Landing page', open: false },
  { label: 'Unattributed visit', open: true },
];
...
export function PopulationToggle() {
  ...
  const aim = useCallback(() => {
    ...
    const aimed: { mark: HTMLElement; tx: number; ty: number; delay: number }[] = [];
    let visible = 0;
    for (const mark of marks) {
      const width = mark.offsetWidth;
      const height = mark.offsetHeight;
      if (width === 0 && height === 0) continue; // hidden at NARROW
      const target = targets[visible % targets.length];
      const cx = mark.offsetLeft + width / 2;
      const cy = mark.offsetTop + height / 2;
      aimed.push({
        mark,
        tx: Math.round(target.x - cx),
        ty: Math.round(target.y - cy),
        delay: (visible % 17) * 11,
      });
      visible += 1;
    }
    for (const item of aimed) {
      item.mark.style.setProperty('--rsv-tx', `${item.tx}px`);
      item.mark.style.setProperty('--rsv-ty', `${item.ty}px`);
      item.mark.style.transitionDelay = `${item.delay}ms`;
    }
    ...
    <div className="rsv-stage rsv-rule-t border-t-2 pt-4" data-rsv-stage="">
      <div className="mb-[18px] flex flex-wrap items-baseline justify-between gap-x-[22px] gap-y-3">
        <p
          ...
          aria-live="polite"
        >
          {isTraffic ? 'what an agency usually shows you' : 'what we measure'}
        </p>
      ...
      <p className="rsv-muted mt-4 max-w-none text-[.82rem] font-medium leading-[1.5]">
        We connect the journey from where a buyer found you to the enquiry they made, wherever the
        available data allows it. Illustrative only: the marks are{' '}
        <b className="rsv-strong font-bold">kinds, not counts</b>, and no quantity, proportion or rate
        is drawn or claimed.
      </p>
    </div>
  );
