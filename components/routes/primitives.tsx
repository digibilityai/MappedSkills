...
export function RouteHero({
  ...
  return (
    <section className="bg-resolve-ground pt-[70px] pb-[65px] text-resolve-ink">
      <Container>
        <div className={cn(OPENER_GRID[effective], 'min-[1081px]:max-w-[70%] min-[1081px]:w-[70%]')}>
            ...
            {effective === 'split' && (
              <h1
                className={cn(
                  ...
                  size === 'full'
                    ? 'max-w-none text-[clamp(2rem,3.4vw,3.1rem)]'
                    : 'max-w-none text-[clamp(1.85rem,3vw,2.6rem)]'
                )}
              >
                {title}
              </h1>
          ...
          <div className={OPENER_MAIN[effective]}>
            {effective !== 'split' && (
              <h1
                className={cn(
                  ...
                  size === 'full'
                    ? 'max-w-none text-[clamp(2rem,4.6vw,3.9rem)]'
                    : 'max-w-none text-[clamp(1.85rem,3.7vw,3.1rem)]',
            ...
            {(lede || children) && (
              <div
                className={cn(
                  'mt-[clamp(20px,2.4vw,30px)] max-w-none text-[clamp(1.06rem,1.35vw,1.28rem)] leading-relaxed text-resolve-dim [&>p+p]:mt-4',
    ...
    <dl className="m-0 mt-[clamp(24px,3vw,40px)] border-t-2 border-resolve-ink p-0">
      {items.map((item, i) => (
        <div key={i} className="border-b border-resolve-line py-[clamp(18px,2.2vw,28px)]">
          ...
          <dd className="m-0 mt-3 max-w-none text-[1.02rem] leading-relaxed text-resolve-dim">{item.body}</dd>
        </div>
      ))}
    </dl>
  );
...
export function StatedList({ items, className }: { items: ReactNode[]; className?: string }) {
  return (
    <ul className={cn('m-0 mt-[clamp(20px,2.4vw,32px)] list-none border-t border-resolve-line p-0', className)}>
      {items.map((item, i) => (
        <li key={i} className="max-w-none border-b border-resolve-line py-[clamp(12px,1.6vw,18px)] text-[1.02rem] leading-relaxed">
          {item}
        </li>
      ))}
    </ul>
...
export function StageSystem({ stages, caption }: { stages: Stage[]; caption: ReactNode }) {
  return (
    <figure className="m-0 mt-[clamp(26px,3.2vw,44px)]">
      ...
      <figcaption className="mt-[clamp(18px,2.2vw,26px)] max-w-none text-[.9rem] leading-[1.55] text-resolve-dim">
        {caption}
      </figcaption>
    </figure>
  );
