...
| **"There are buyers for what you sell, and most never reach you"** | Opened both `/` and `/services` in almost the same words | Left deliberately on both — it is the Level 1–2 message and the two pages are a step apart in one journey — **but flagged for the design phase**, because identical openings on consecutive pages read as a template even when the strategy is correct |
